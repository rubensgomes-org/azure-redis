...
`azure-redis` was scaffolded from `spring-blueprint` as build/CI/release machinery
only. This file records what was deliberately left outstanding so the scaffold
could be handed over green.

## Outstanding

- [ ] Add the Azure dependency — spring-boot-starter-data-redis with Azure Cache for Redis — to `app/build.gradle.kts`,
...
- [ ] Build the real endpoint that demonstrates Azure Cache for Redis.
...
- [ ] Create the SonarCloud project `rubensgomes-org_azure-redis` in the
