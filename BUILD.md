...
How the Gradle build for `azure-redis` is put together, what each task
...
> **Note** — `logging.level.com.rubensgomes.azure.redis` is set to `trace` in the
...
```bash
kill $(pgrep -f com.rubensgomes.azure.redis.App)
```
...
```bash
./gradlew bootJar
java -jar app/build/libs/azure-redis-<version>.jar
```
...
```kotlin
excludes = listOf("com.rubensgomes.azure.redis.config.*")
```
...
| Archive                                  | Contents                                                 |
|------------------------------------------|----------------------------------------------------------|
| `azure-redis-<version>.jar`         | executable Spring Boot jar (layered, for Docker caching) |
| `azure-redis-<version>-plain.jar`   | library jar, classes only                                |
| `azure-redis-<version>-sources.jar` | sources                                                  |
| `azure-redis-<version>-javadoc.jar` | Javadoc                                                  |
...
| `sonar.projectKey`       | `rubensgomes-org_azure-redis` |
| `sonar.projectName`      | `azure-redis`                 |
...
there is no `azure-redis-spring-boot.jar` for the Dockerfile and no `.env`.
